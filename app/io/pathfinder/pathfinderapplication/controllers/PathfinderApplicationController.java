package io.pathfinder.pathfinderapplication.controllers;
...
public class PathfinderApplicationController extends Controller {
  ...
  public Result createApplication() {
    ...
    if (!jsonNode.isObject()) {
      return badRequest("Json sent was not a Json objects");
    }

    int size = jsonNode.size();
    if (size > PathfinderApplication.REQUIRED_CREATE_FIELDS) {
      return badRequest("Too many fields were detected in the application create request");
    }

    if (size < PathfinderApplication.REQUIRED_CREATE_FIELDS) {
      return badRequest("Too few fields were detected in the application create request");
    }

    if(!jsonNode.has("name")) {
      return badRequest("Json did not include a name field");
    }
